...
function Write-HelloMessage {
    ...
    process {
        ...
        Write-Center -Message 'Starting the Spicetify spotifyx script...' -ForegroundColor Blue
        Write-Host
        Write-Center -Message 'github.com/yafyx/spotifyx' -ForegroundColor Blue
        Write-Center -Message '----------------------------------------' -ForegroundColor Blue
        Write-Host
    }
}
...
function Write-ByeMessage {
    ...
    process {
        ...
        Write-Host
        Write-Center -Message 'Thanks for using spotifyx!' -ForegroundColor Green
        Write-Center -Message '----------------------------------------' -ForegroundColor Green
        Write-Host
    }
}
...
function Get-SpicetifyFoldersPaths {
    ...
    process {
        @{
            ...
            spotifyxPath  = "$(spicetify path userdata)\Themes\spotifyx"
        }
    }
}
...
#region spotifyx
function Get-spotifyx {
    [CmdletBinding()]
    [OutputType([string])]
    param ()
    begin {
        $archiveName = 'spotifyx-main'
        $Temp = [System.IO.Path]::GetTempPath()
        $archivePath = "$Temp\$archiveName.zip"
    }
    process {
        Write-Verbose -Message 'Downloading the spotifyx repository archive...' -Verbose
        $Parameters = @{
            UseBasicParsing = $true
            Uri             = 'https://codeload.github.com/yafyx/spotifyx/zip/refs/heads/main'
            OutFile         = $archivePath
        }
        Invoke-WebRequest @Parameters
        
        Write-Verbose -Message 'Unpacking the spotifyx repository archive...' -Verbose
        $Parameters = @{
            Path            = $archivePath
...
function Install-spotifyx {
    ...
    begin {
        Write-Verbose -Message 'Installing spotifyx theme...' -Verbose
        $spotifyxSrcPath = "$Path\src"
        $spotifyxRemotePath = "$Path\remote"
    }
    process {
        ...
        if ($Type -eq 'Remote') {
            $Parameters = @{
                Path        = "$spotifyxSrcPath\color.ini"
...
function Uninstall-spotifyx {
    ...
    begin {
        Write-Verbose -Message 'Uninstalling spotifyx theme...' -Verbose
    }
    process {
